...
_INLINE_ENGINE_KWARGS = {
    "custom_pipeline_args": {"pipeline_class": CUSTOM_PIPELINE_CLASS},
    "worker_extension_cls": WORKER_EXTENSION_CLASS,
    "enforce_eager": True,
}
_INLINE_PARAMS = [pytest.param(AsyncOmniParams(model=MODEL, extra_omni_kwargs=_INLINE_ENGINE_KWARGS), id="inline")]
_SLEEP_PARAMS = [
    pytest.param(
        AsyncOmniParams(model=MODEL, extra_omni_kwargs={**_INLINE_ENGINE_KWARGS, "enable_sleep_mode": True}),
        id="inline-sleep",
    )
]
